...
HRESULT CD3DXSKINMESH::Init(CD3DXSKINMESH_INIT* si)
{
	...
	SetShaderDirectory();
	//ブロブからバーテックスシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"SKIN.hlsl",NULL,NULL,"VSSkin","vs_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
	...
	//ブロブからピクセルシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"SKIN.hlsl",NULL,NULL,"PSSkin","ps_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
		MessageBox(0,L"hlsl読み込み失敗",NULL,MB_OK);
		return E_FAIL;
	}
